...
Given an {\sf L}-specification $G = (G_1, \ldots, G_n)$ of  a graph $E(G)$,
...
given an {\sf L}-specification $F = (F_1, \ldots, F_n)$ of a {\sf CNF}
...
polynomial time.
The correctness of the algorithm and its running time are addressed in
...
Step 1(a) uses $O(n_i^2)$ time since the size of each 
...
In the $i^{th}$ iteration, Step 1(b) uses less time than Step 1(a).
Also, in the $i^{th}$ iteration, Step 1(c) uses $O(n_i^3)$ time and
Step 1(d) uses $O(n_i^3)$ time.
Hence, the $i^{th}$ iteration uses $O(n_i^3)$ time. 
Hence the total time used by the algorithm is given by
\[ \sum_{i=1}^n n_i^3 = O(N^3). \]
In other words, the algorithm runs in $O(N^3)$ time. \hfill\QED
