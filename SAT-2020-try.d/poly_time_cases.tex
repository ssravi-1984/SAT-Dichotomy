...
method for processing hierarchicall specified graphs discussed 
...
the bottom up method processes the non-terminal $G_i$ in the $ith$ iteration
...
In this paper, we will consider polynomial time alorithms for solving
various satisfiability problems specified using {\sf L}- or 
{\sf 1-FPN}-specifications. As in the case of {\sf L}-specified graphs, 
our algorithms process the speification in a bottom up fashion. Specifically,
...
\noindent
{\bf Remark:} Observe that $F_1$ does not calll any non-teminal. 
...
\noindent
{\bf Renmark:} The variables of $F^b_i$ are the input variables of $F_i$
...
lemma about representing every 1-weakly postive or 1-weakly negative relation
as a 1-weakly postive or a 1-weakly negative formula respectively.
...
1 unnegated literal.
Observe that by preceeding discussion it is clear that 
...
its correpsonding burnt formula is a 1-weakly positive {\sf CNF} formula 
...
(i) the definition of 3CNF weakly postive formula, and 
...
2(a), 2(b) or 2(c) exceuted by the algorithm.
...
occurs as a single literal postive as well as negative clause, then the
...
Before, we discuss the proof we point out the intuitive differrence between the
...
The intuitive reson for this contrast is to do with the semantics of 
{\em implication}. 
