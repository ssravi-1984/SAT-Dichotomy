...
\begin{enumerate}
\item
1-FPN-3SAT is in NSPACE(n).
\item
There is a $\theta(n)$ size 
...
\noindent
{\bf Proof:} The {\sf PSPACE}-hardness of {\sf 1-FPN-3SAT(BC)} follows 
...
deterministic space les than $O(n^2)$. 
This would improveme Savitch's \cite{Sa70} 
...
most one unnegated variable in each conjunct. Alos recall that,
